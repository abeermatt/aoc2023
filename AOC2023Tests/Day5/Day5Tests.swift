//
//  Day4Tests.swift
//  AOC2023Tests
//
//  Created by Matthew Hobbs on 04/12/2023.
//

import XCTest

final class Day5Tests: XCTestCase {
    
    let sampleInput = """
seeds: 79 14 55 13

seed-to-soil map:
50 98 2
52 50 48

soil-to-fertilizer map:
0 15 37
37 52 2
39 0 15

fertilizer-to-water map:
49 53 8
0 11 42
42 0 7
57 7 4

water-to-light map:
88 18 7
18 25 70

light-to-temperature map:
45 77 23
81 45 19
68 64 13

temperature-to-humidity map:
0 69 1
1 0 69

humidity-to-location map:
60 56 37
56 93 4
"""

    func xtestPart1Sample() throws {
        XCTAssertEqual(35, Day5.Part1.lowestLocation(sampleInput))
    }

    func xtestPart1() throws {
        let input = try TestHelpers.readFile(named: "Day5")

        XCTAssertEqual(265018614, Day5.Part1.lowestLocation(input))

    }

    func xtestPart2Sample() async throws {
        let result = await Day5.Part2.lowestLocation(sampleInput)
        XCTAssertEqual(46, result)

    }
    
    
    func xtestPart2() async throws {
        let input = try TestHelpers.readFile(named: "Day5")
        let result = await Day5.Part2.lowestLocation(input)
        XCTAssertEqual(63179500, result)

    }


}
